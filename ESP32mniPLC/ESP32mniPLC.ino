...
Output Outputs[6];
uint8_t outs[6] = { 19, 23,18, 5, 17, 16 };
int dataLenght;
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len)
 {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) 
  {
    ...
    if (payload[0] == '&') 
    {
    debug_data += "message lenght: " + String(len) +">>";
      ...
      if (payload[1] == 'T')
      {
        ...
        debug_data += tstamp + String(rtc_counter);
        return;
      }
      debug_data += "\ndata: " + String(rec_data) + "\n" + "sub:\n ";
      String sub = "";
      // tabulka termostatu
    ...
    {
      debug_data += "IO " + String(payload[3]) + String(payload[3]);
      switch (payload[3]) 
      {
        case '1':
          if (payload[5] == 'n')
...
//vystupy spinacek a termostatu
  ...
  String IOstates = "#IST*";
  for(int i=0; i<4;i++){IOstates += String(Inputs[i].last_state);}
  IOstates += "*";
  IOstates += "#OST*";
  for(int i=0; i<6;i++){IOstates += String(Outputs[i].last_state);}
  IOstates += "*END";  
...
#endif
  delay(500);
  notifyClients(String(msg)+IOstates);
  delay(500);  
